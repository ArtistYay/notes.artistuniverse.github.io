---
tags:
  - Cybersecurity
  - Defensive Security
---

*What can I see?* Depending on the operating system (OS) I can see who logs in to the computer, who started a process, who ran a command, what command was ran, etc. For example, in Windows OS you can view logs in the '*Event Viewer*'

[Windows Security Log Events](https://www.ultimatewindowssecurity.com/securitylog/encyclopedia/default.aspx)

*Why is that important?* To make sure no malicious behavior is seen on that machine/computer. Attackers use different ways to recon, exploit, and extract.

![screenshot](../../../images/cyber_kill_chain.png)
